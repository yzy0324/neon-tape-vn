...
- 4 名主要角色（主角 + 3 NPC）使用 SVG 像素风赛博朋克立绘，固定渲染尺寸 96x144。
- 立绘采用统一调色板（霓虹紫/霓虹青/霓虹粉 + 暗背景），每个角色提供 neutral / smile / angry 三种表情。
- 对话推进时会依据 speaker 与 scene expression 自动切换立绘，并附带轻微扫描线闪烁动画。
